            ...
            for sub_training_loop in range(sub_loops):
                ...
                for t in range(timesteps):
                    ...
                    if np.random.rand() <= epsilon:
                        ...
                        #Bellman double Q
                        Q = model.compute_interaction_effect(diff_obs[np.newaxis, :])          # Q-values predictions

                        Q_ = model.compute_interaction_effect(diff_obs_new[np.newaxis, :])
                        Q_target = target_model.compute_interaction_effect(diff_obs_new[np.newaxis, :])
                ...
                if len(memory) >= mb_size:
                    ...
                    for i in range(0, mb_size):
                        ...
                        Q = model.compute_interaction_effect(diff_obs[np.newaxis, :])
                        Q_ = model.compute_interaction_effect(diff_obs_new[np.newaxis, :])
                        Q_target = target_model.compute_interaction_effect(diff_obs_new[np.newaxis, :])
