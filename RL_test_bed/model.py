...
from RL.utilities import find_trainable_variables
...
class Model(object):
    ...
    def __init__(self,
        ...
        vf_loss = tf.losses.mean_squared_error(rewards_, tf.squeeze(train_model.vf))
        ...
        def load(load_path, ckpt_name="ckpt.ckpt"):
            """
            Load the model
            """
            if not os.path.exists(load_path):
                os.makedirs(load_path)
            if tf.train.checkpoint_exists(load_path + ckpt_name):
                saver = tf.train.Saver()
                print('Loading ' + load_path + ckpt_name)
                saver.restore(sess, load_path + ckpt_name)
